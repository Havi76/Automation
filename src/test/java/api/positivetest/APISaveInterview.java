...
@Story("הוספת ראיון")
@Severity(CRITICAL)
@Test(groups = {"API positive test"}, suiteName = "api", description = "API for adding interview")
public class APISaveInterview {
    final String BASE_URL = "https://mefakdim-backend.azurewebsites.net";
    final String REQUEST_URN = "/interviews/saveInterview";
    ...
    void sendDistributionMail(){
        ...
        Response response = given().log().all().contentType(ContentType.JSON)
                .body(new File("src/test/java/api/jsonsfile/SaveInterview.json"))
